...
3. Generate commit message using **Conventional Commits** format:
   ...
   - ⚠️ **MANDATORY — NEVER OMIT:** Every commit message MUST end with the following attribution block. This is non-negotiable:
     ...
     **Attribution by tool:**
     | Tool | Co-Authored-By |
     |------|----------------|
     | Claude Code | `Claude <ModelName> <noreply@anthropic.com>` (e.g., `Claude Opus 4.5`) |
...
- ⚠️ **MANDATORY — NEVER OMIT:** Every PR description MUST end with the same attribution block as commits:
...
- ⚠️ **MANDATORY — NEVER OMIT:** Every PR comment (review comments, inline comments, general comments) MUST include the following disclosure. NEVER post a comment on my behalf without it:
  ...
  ```
  > 🤖 This comment was generated by an AI coding agent.
  ```
