...
" This file is only loaded for Neovim (has('nvim') check in main.vim)

if has('nvim')
  lua << EOF
  -- Context-aware right-click menu function
  _G.show_context_menu = function()
    local menu_items = {}
    local actions = {}
    ...
    if not _G._git_repo_cache then
      _G._git_repo_cache = {}
    end
    local cwd = vim.fn.getcwd()
    if _G._git_repo_cache[cwd] == nil then
      local result = vim.fn.system("git rev-parse --is-inside-work-tree 2>/dev/null")
      _G._git_repo_cache[cwd] = result:match("true") ~= nil
    end
    local in_git_repo = _G._git_repo_cache[cwd]
    ...
    if has_lsp then
      ...
      table.insert(actions, function() vim.lsp.buf.definition() end)

      table.insert(menu_items, "  Find References")
      table.insert(actions, function() vim.lsp.buf.references() end)

      table.insert(menu_items, "  Rename Symbol")
      table.insert(actions, function() vim.lsp.buf.rename() end)

      table.insert(menu_items, "  Code Actions")
      table.insert(actions, function() vim.lsp.buf.code_action() end)

      table.insert(menu_items, "  Show Hover Info")
      table.insert(actions, function() vim.lsp.buf.hover() end)
    end
    ...
    if has_diagnostics then
      ...
      table.insert(actions, function() vim.diagnostic.open_float() end)

      table.insert(menu_items, "  Next Diagnostic")
      table.insert(actions, function() vim.diagnostic.goto_next() end)

      table.insert(menu_items, "  Previous Diagnostic")
      table.insert(actions, function() vim.diagnostic.goto_prev() end)
    end
    ...
    if in_git_repo then
      ...
      table.insert(actions, function() vim.cmd("Gitsigns blame_line") end)

      table.insert(menu_items, "  Preview Hunk")
      table.insert(actions, function() vim.cmd("Gitsigns preview_hunk") end)

      table.insert(menu_items, "  Stage Hunk")
      table.insert(actions, function() vim.cmd("Gitsigns stage_hunk") end)

      table.insert(menu_items, "  Reset Hunk")
      table.insert(actions, function() vim.cmd("Gitsigns reset_hunk") end)

      table.insert(menu_items, "  Diff This")
      table.insert(actions, function() vim.cmd("Gitsigns diffthis") end)
    end
    ...
    table.insert(actions, function() vim.cmd("write") end)

    table.insert(menu_items, "  Save All")
    table.insert(actions, function() vim.cmd("wall") end)
    ...
    if is_visual then
      ...
      table.insert(actions, function() vim.cmd('normal! yy') end)

      table.insert(menu_items, "  Delete Line")
      table.insert(actions, function() vim.cmd('normal! dd') end)
    end

    table.insert(menu_items, "  Paste")
    table.insert(actions, function() vim.cmd('normal! p') end)

    table.insert(menu_items, "  Select All")
    table.insert(actions, function() vim.cmd('normal! ggVG') end)

    table.insert(menu_items, "  Undo")
    table.insert(actions, function() vim.cmd('undo') end)

    table.insert(menu_items, "  Redo")
    table.insert(actions, function() vim.cmd('redo') end)
